...
const SECP_GEN: SecP256Affine = SecP256Affine::new_unchecked(G_GENERATOR_X, G_GENERATOR_Y);
...
pub struct PoKEcdsaSigCommittedPublicKeyProtocol<const NUM_REPS_SCALAR_MULT: usize = 128> {
    ...
    pub protocol_minus_zR:
        ScalarMultiplicationProtocol<SecP256Config, Tom256Config, NUM_REPS_SCALAR_MULT>,
    /// Protocol for relation `-g*t*r^-1 = q + z*(-R)`
    pub protocol_add: PointAdditionProtocol<SecP256Config, Tom256Config>,
}
...
pub struct PoKEcdsaSigCommittedPublicKey<const NUM_REPS_SCALAR_MULT: usize = 128> {
    ...
    pub proof_minus_zR:
        ScalarMultiplicationProof<SecP256Config, Tom256Config, NUM_REPS_SCALAR_MULT>,
    /// Proof of relation `-g*t*r^-1 = q + z*(-R)`
    pub proof_add: PointAdditionProof<SecP256Config, Tom256Config>,
}

impl TransformedEcdsaSig {
    pub fn new(
        sig: &ecdsa::Signature,
        hashed_message: SecP256Fr,
        public_key: SecP256Affine,
    ) -> Result<Self, Error> {
        let s_inv = sig
            .response
            .inverse()
    ...
    pub fn r_inv(R: &SecP256Affine) -> Result<SecP256Fr, Error> {
        SecP256Fr::from(R.x.into_bigint())
            .inverse()
            .ok_or(Error::EcdsaSigResponseNotInvertible)
    }
}
    ...
    PoKEcdsaSigCommittedPublicKeyProtocol<NUM_REPS_SCALAR_MULT>
{
    ...
    pub fn init<R: RngCore>(
        ...
        let protocol_minus_zR = ScalarMultiplicationProtocol::<
            SecP256Config,
            Tom256Config,
            NUM_REPS_SCALAR_MULT,
        >::init(
            rng,
            comm_z.clone(),
            comm_minus_zR.clone(),
...
impl<const NUM_REPS_SCALAR_MULT: usize> PoKEcdsaSigCommittedPublicKey<NUM_REPS_SCALAR_MULT> {
    ...
    pub fn verify_using_randomized_mult_checker(
        ...
        comm_key_secp: PedersenCommitmentKey<SecP256Affine>,
        comm_key_tom: PedersenCommitmentKey<Tom256Affine>,
        rmc_1: &mut RandomizedMultChecker<SecP256Affine>,
...
mod tests {
    ...
    fn transformed_sig_verify() {
        ...
        let sk = SecP256Fr::rand(&mut rng);
        let pk = (SECP_GEN * sk).into_affine();

        let message = SecP256Fr::rand(&mut rng);
    ...
    fn pok_ecdsa_sig_comm_pubkey() {
        ...
        let comm_key_secp = PedersenCommitmentKey::<SecP256Affine>::new::<Blake2b512>(b"test1");
        ...
        for i in 0..num_iters {
            ...
            let mut checker_1 = RandomizedMultChecker::<SecP256Affine>::new_using_rng(&mut rng);
